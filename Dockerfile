...
ENV STACK_BIN=https://github.com/commercialhaskell/stack/releases/download/v2.9.1/stack-2.9.1-linux-x86_64-bin
...
COPY stack-2.9.1-linux-x86_64-bin.sha256 \
     stack-2.9.1-linux-x86_64-bin.asc \
     GPG-KEY-575159689BEFB442-dev@fpcomplete \
     ./
# hadolint ignore=DL4006
RUN curl -fsSL "$STACK_BIN" -o ${STACK_BIN##*/} && \
    sha256sum -c stack-2.9.1-linux-x86_64-bin.sha256 >&2 && \
    gpg -q --import GPG-KEY-* && \
    printf "trust\n5\ny\n" | gpg --command-fd 0 --no-tty --edit-key 575159689BEFB442 2>/dev/null && \
    gpg --verify stack-2.9.1-linux-x86_64-bin.asc && \
    rm -rf ~/.gnupg && \
...
ENV PATH=/usr/bin:/bin:/usr/local/bin:/home/builder/bin:/home/builder/.local/bin:/home/builder/.stack/programs/x86_64-linux/ghc-tinfo6-9.0.2/bin
